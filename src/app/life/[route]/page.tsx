...
async function loadLifeMdx(route: string) {
  const safeRoute = route.replace(/[^a-zA-Z0-9-]/g, "")
  const filePath = path.join(process.cwd(), "src", "content", "life", `${safeRoute}.mdx`)
  ...
  return compileMDX<LifePageFrontmatter>({
    source,
    options: {
...
export async function generateStaticParams() {
  const dirPath = path.join(process.cwd(), "src", "content", "life")
...
export default async function LifeRoutePage({
  params,
}: {
  params: Promise<{ route: string }>
}) {
  const { route } = await params
  const { content, frontmatter } = await loadLifeMdx(route)
    ...
    <div className="page-enter pt-16 md:pt-24">
      <section className="max-w-5xl mx-auto px-6 py-16 md:py-24">
        <div className="mb-10 md:mb-14">
          <span className="text-xs inline-block font-sans font-medium uppercase tracking-widest text-gold mb-3">life</span>
