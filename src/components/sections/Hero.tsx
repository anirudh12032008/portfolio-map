...
function Rot(){
    ...
    return(
        <span className="relative inline-flex overflow-hidden align-baseline ml-1.5" style={{ minWidth: "10ch" }}>
            <motion.span
                key={builds[i]}
                initial={reduce ? false : { y: "70%", opacity: 0 }}
                animate={{ y: "0%", opacity: 1 }}
                transition={{ duration: 0.5, ease: [0.25, 0.1, 0.25, 1] }}
                className="font-serif italic text-navy inline-block text-[1.35em] leading-none"
            >
                {builds[i]}
            </motion.span>
        </span>
